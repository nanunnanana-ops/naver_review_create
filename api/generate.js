 ...
 *
 * 무료 옵션:
 * 1. Google Gemini API - (권장)
 * 2. Hugging Face Inference API - (제한적)
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (geminiKey) {
      ...
      reviews = await generateWithGemini(
        menuText,
        sideText,
        keywordsText,
        storeName,
      ...
      reviews = await generateWithHuggingFace(
        menuText,
        sideText,
        keywordsText,
        storeName,
...
자연스럽고 진짜 손님이 쓴 것 같은 말투로 3개의 서로 다른 리뷰를 작성해주세요.`;
  ...
  try {
    ...
    return reviews.length >= 3
      ? reviews
      : [`${menuText} 먹었는데 맛있었어요.`, `${menuText} 주문했어요. 좋았습니다.`, `${menuText} 시켰는데 괜찮았어요.`];
  } catch (error) {
